...
@app.get("/")
def root():
    return {"message": "Congressional App Challenge API is running!"}
...
def get_surveys():
    try:
        ...
        result = supabase.table("surveys").select("*").order("title").order("question_number").execute()
...
def get_results(survey_title: str, school_id: int):
 ...
 #Get data for and set up leaderboard   

@app.get("/leaderboard/{survey_title}")
def get_leaderboard(survey_title: str):
    try:
        # Get survey ID from title
        survey_result = supabase.table("surveys").select("id").eq("title", survey_title).eq("question_number", 1).execute()
        ...
        responses = supabase.table("responses").select("school_id").eq("survey_id", survey_id).execute()
        ...
        # Build leaderboard with school names
        leaderboard = []
        for school in schools.data:
            count = school_counts.get(school["id"], 0)
            if count > 0:  # Only include schools with responses
                leaderboard.append({
        ...
        leaderboard.sort(key=lambda x: x["response_count"], reverse=True)
